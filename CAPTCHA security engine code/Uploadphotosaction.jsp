...
%>
<%
String file=request.getParameter("uploadphoto");
...
newPath="C:\\Program Files\\Apache Software Foundation\\Tomcat 6.0\\webapps\\CAPTCHA\\photos\\"+arr.get(count-1);
int c;
...
try
{
	String files=request.getParameter("uploadphoto");
    ...
    Class.forName("oracle.jdbc.driver.OracleDriver");
     ...
     ps=con.prepareStatement("insert into UploadPhoto  values(?, ?,?,?) ");
 ...
 ps.setString(1,usernamesession);
   ps.setBinaryStream(2,fi,(int)f.length());

  ps.setString(3,fname);
  ps.setString(4,capcode);
    int i=ps.executeUpdate();
   if(i>0)
	{
	 //out.println("success");
	response.sendRedirect("UploadPhotoRetrieve.jsp");
	}
	else
	{
	  out.println("failure");
